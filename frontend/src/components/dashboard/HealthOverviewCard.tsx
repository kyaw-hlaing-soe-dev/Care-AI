...
export function HealthOverviewCard({ record }: { record: VitalRecord }) {
  ...
  return (
    <GlassCard strong className="app-card p-5 sm:p-6 lg:p-7">
      <div className="grid items-center gap-5 sm:grid-cols-[auto_1fr] lg:grid-cols-[auto_minmax(210px,.8fr)_minmax(280px,1.2fr)] lg:gap-6">
        <HealthScoreRing score={record.analysis.score} hasData size={112} />
        ...
        <div className="min-w-0">
          ...
          <p className="mt-3 text-xs text-slate-500">{t("dashboard.updated", { date: formatRecordedAt(record.recordedAt) })}</p>
        </div>

        <div className="rounded-[18px] border border-blue-100/80 bg-gradient-to-br from-blue-50/70 to-cyan-50/55 p-4 sm:col-span-2 lg:col-span-1">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              >
                {t("dashboard.viewFullInsight")} <ArrowRight className="size-4" aria-hidden="true" />
              </a>
            </div>
          </div>
        </div>
