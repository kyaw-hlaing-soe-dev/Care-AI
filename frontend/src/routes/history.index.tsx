...
import { useHistory } from "@/hooks/use-vitals";
  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Health History — CareAI" },
      {
        ...
        content: "Review your readings and CareAI insights over time.",
      },
...
function HistoryPage() {
  ...
  const period = filter === "7" ? "7d" : filter === "30" ? "30d" : "all";
  const {
    records,
    isLoading: loading,
    error,
  ...
  if (error) {
    return (
      <GlassCard className="app-card p-8 text-center">
        <p className="text-sm text-slate-500">We couldn't load your history. Try again.</p>
      </GlassCard>
    );
  }
  ...
  const averageScore = records.length
    ? Math.round(
        records.reduce((total, record) => total + record.analysis.score, 0) / records.length,
      )
    : 0;
    ...
    <div className="mx-auto max-w-[1120px] space-y-7 lg:space-y-8">
      ...
      {records.length === 0 ? (
        <GlassCard strong className="app-card px-5 py-12 text-center sm:px-8 sm:py-16">
          ...
          <h2 className="app-section-title mt-5 text-2xl font-extrabold tracking-[-0.035em] text-slate-950">
            {t("history.noHistory")}
          </h2>
        ...
        <>
          ...
          >
            {[
              ...
              {
                label: t("history.latestReading"),
                value: records[0] ? formatRecordedAt(records[0].recordedAt) : "—",
                Icon: Clock3,
              },
              ...
              <GlassCard key={label} delay={index * 45} className="app-card min-w-0 p-4 sm:p-5">
                ...
                <p
                  className={cn(
                    "mt-1 font-extrabold tracking-[-0.025em] text-slate-950",
                    label === t("history.latestReading") ? "text-sm leading-5" : "text-2xl",
                  )}
                >
                  {value}
                </p>
          ...
          <section aria-labelledby="timeline-heading">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div
                className="inline-flex w-full rounded-[15px] border border-slate-200 bg-white/75 p-1 sm:w-auto"
                role="group"
