# CareAI Definition of Done

**Status:** Active  
**Version:** 1.0  
**Last Updated:** 2026-08-08  
**Related:** [QA Checklist](../13-testing/qa-checklist.md), [Coding Rules](./coding-rules.md)

A feature is not done merely because code exists. Confirm:

- [ ] Requirement and acceptance criteria implemented
- [ ] Client and server validation where applicable
- [ ] Loading, empty, success, and safe error states
- [ ] Authorization/security and secret handling reviewed
- [ ] Responsive and accessibility checks completed
- [ ] No new console errors; tests/build pass
- [ ] No private data in public marketing, URL, logs, or browser storage contrary to spec
- [ ] Data/schema/API changes documented and relevant spec status updated
